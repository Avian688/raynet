...
class OmnetGymApiEnv(gym.Env):
    def __init__(self,env_config):
        ...
        self.action_space = spaces.Box(low=0.0, high=2.0, shape=(1,), dtype=np.float32) # A single float value ranging from 0.0 -> 2.0

        # Define the observation space (expected values/types for each observation feature)
        low_bounds = [0, 0, 0, 0,]
        high_bounds= [np.finfo(np.float32).max, 10, np.finfo(np.float32).max,  np.finfo(np.float32).max,]
        low_bounds = np.array(low_bounds, dtype=np.float32)
        high_bounds = np.array(high_bounds, dtype=np.float32)
        self.observation_space = spaces.Box(low=low_bounds, high=high_bounds, dtype=np.float32) # A 4-dimensional array, each feature is a float value with its own bounds
    ...
    def step(self, actions):
        ...
        obs = np.asarray(list(obs['JamesCC']), dtype=np.float32)    # also formats the RLAgent's obs so RLlib can understand it
        reward = round(rewards['JamesCC'], 4)                                 # Get the reward our RLAgent is reporting
        sim_truncated = False                                       # Assume the simulation is still running (for now)
